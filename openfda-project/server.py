...
class testHTTPRequestHandler(http.server.BaseHTTPRequestHandler):
    ...
    def resultados_genericos (self, limit=10):
        ...
        respuesta = resp1.read().decode("utf8")
    ...
    def do_GET(self):
        ...
        if self.path=='/': #Este recurso tiene 5 formularios
            ...
            resultado_html = self.pagina_web (medicamentos_lista)
            ...
            resultado_html = self.pagina_web(companies_lista)
            self.wfile.write(bytes(resultado_html, "utf8"))
            ...
            resultado_html = self.pagina_web(warnings_lista)
            self.wfile.write(bytes(resultado_html, "utf8"))
            ...
            for resultado in search_drug:
                if ('generic_name' in resultado['openfda']):
                    drugs_lista.append(resultado['openfda']['generic_name'][0])
                else:
                    drugs_lista.append('Medicamento desconocido')
            resultado_html = self.pagina_web(drugs_lista)
            self.wfile.write(bytes(resultado_html, "utf8"))
            ...
            resultado_html = self.pagina_web(companies_lista1)
            self.wfile.write(bytes(resultado_html, "utf8"))
